...
use client::{backend::Backend, runtime_api::BlockT, BlockchainEvents};
...
pub use components::{
	CodeExecutor, ComponentBlock, ComponentClient, ComponentExHash, ComponentExtrinsic,
	ComponentOffchainStorage, Components, FactoryBlock, FactoryChainSpec, FactoryExtrinsic,
	FactoryFullConfiguration, FactoryGenesis, FullBackend, FullClient, FullComponents,
	FullExecutor, InitialSessionKeys, LightBackend, LightClient, LightComponents, LightExecutor,
	NetworkService, PoolApi, RuntimeGenesis, ServiceFactory,
};
...
pub struct Service<Components: components::Components> {
	...
	network_status_sinks: Arc<
		Mutex<
			Vec<mpsc::UnboundedSender<(NetworkStatus<ComponentBlock<Components>>, NetworkState)>>,
		>,
	>,
...
pub fn new_client<Factory: components::ServiceFactory>(
	...
	components::FullComponents::<Factory>::build_client(config, executor, None).map(|r| r.0)
}
...
impl<Components: components::Components> Service<Components> {
	...
	pub fn new(
		...
		let (client, on_demand) =
			Components::build_client(&config, executor, Some(keystore.clone()))?;
		...
		let (netstat_tx, netstat_rx) =
			mpsc::unbounded::<(NetworkStatus<ComponentBlock<Components>>, NetworkState)>();
		network_status_sinks.lock().push(netstat_tx);
		let tel_task = netstat_rx
			.for_each(move |(net_status, network_state)| {
		...
		let telemetry = config.telemetry_endpoints.clone().map(|endpoints| {
			...
			let _ =
				to_spawn_tx.unbounded_send(Box::new(future.select(exit.clone()).then(|_| Ok(()))));
			telemetry
		});
	...
	pub fn network_status(
		&self,
	) -> mpsc::UnboundedReceiver<(NetworkStatus<ComponentBlock<Components>>, NetworkState)> {
		let (sink, stream) = mpsc::unbounded();
...
impl<Components> Future for Service<Components>
where
...
impl<Components> Executor<Box<dyn Future<Item = (), Error = ()> + Send>> for Service<Components>
where
	Components: components::Components,
{
	fn execute(
...
>(
	...
	futures::future::poll_fn(move || {
		...
		while let Ok(Async::Ready(_)) = status_interval.poll() {
			...
			status_sinks
				.lock()
				.retain(|sink| sink.unbounded_send((status.clone(), state.clone())).is_ok());
		}
...
fn transactions_to_propagate<PoolApi, B, H, E>(
	pool: &TransactionPool<PoolApi>,
) -> Vec<(H, B::Extrinsic)>
where
...
impl<B, H, C, PoolApi, E> network::TransactionPool<H, B>
	for TransactionPoolAdapter<C, TransactionPool<PoolApi>>
where
				...
				match self.pool.submit_one(&best_block_id, uxt) {
					...
					Err(e) => match e.into_pool_error() {
						Ok(txpool::error::Error::AlreadyImported(hash)) => {
							hash.downcast::<H>().ok().map(|x| x.as_ref().clone())
						}
						Ok(e) => {
...
mod tests {
	...
	fn should_not_propagate_transactions_that_are_marked_as_such() {
		...
		pool.submit_one(&BlockId::hash(best.hash()), transaction.clone())
			.unwrap();
		pool.submit_one(&BlockId::hash(best.hash()), Extrinsic::IncludeData(vec![1]))
			.unwrap();
